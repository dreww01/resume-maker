...
# Rate limiter instance (60 req/min per IP)
rate_limiter = TokenBucketRateLimiter(rate_per_minute=60)

# Register security and size limit middlewares
# In Starlette, middleware added first executes innermost; middleware added last executes outermost.
# Adding CORS, then ContentSizeLimit, then SecurityHeaders ensures SecurityHeaders runs outermost
# and adds headers to all responses (including 413 responses generated by ContentSizeLimit).
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
