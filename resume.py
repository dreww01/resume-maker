...
def get_multiline_input(prompt):
    """Get multi-line input from user. End with 'END' on its own line."""
    print(prompt)
    print("-" * 50)
...
if __name__ == "__main__":
    ...
    resume_text = read_resume("olisa-resume.pdf")
    print(f"\nResume loaded successfully ({len(resume_text)} characters)")
    ...
    print(f"\nJob description received ({len(job_desc)} characters)")
    print("Processing with AI... please wait...")

    tailored_data = call_openai(resume_text, job_desc)

    output_file = create_docx(tailored_data)
    print(f"\nResume created: {output_file}")
    print("Done!")
    exit(0)
